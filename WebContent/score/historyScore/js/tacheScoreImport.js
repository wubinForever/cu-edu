	...
	var tacheScoreImport = {				
		...
		importExcel : function(){
			var option = {
				...
				uploadUrl: url.IMPORT_TACHE_HISTORY_SCORE,	//导入文件接口
				data : [{name:"学号/姓名",field:"studentNoOrName"},{name:"环节号/环节名称",field:"courseNoOrName"},{name:"导入失败原因",field:"message"}], 	//错误信息显示的字段[{name:"名称",field:"name"}.....]
				exportFailUrl : url.EXPORT_TACHE_HISTORY_SCORE_FAIL_MESSAGE,	//导出错误信息接口路径
				successCallback : function(){return true},	//导入成功后回调函数
				ok : function(){return true},	//点击弹窗确定按钮回调函数
				title : "环节历史成绩导入",	//弹出层显示的内容
				templateUrl : url.TACHE_HISTORY_SCORE_TEMPLATE //下载模板地址
			}
			
			importFileMenu.init(option);
		}
	}
	module.exports = tacheScoreImport;
	window.tacheScoreImport = tacheScoreImport;
});
